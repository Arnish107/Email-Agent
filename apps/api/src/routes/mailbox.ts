...
mailboxRouter.get("/", async (req, res) => {
  const result = await pool.query(
    `SELECT id, provider, email_address, scope, status, last_scan_at, created_at, updated_at
  ...
  res.json({
    providers: [
      ...
      },
      {
        ...
        scope: "Mail.Read",
        configured: false,
        note: "Scaffolded for later; not enabled in this prototype build.",
      },
    ],
  });
});
...
oauthRouter.get("/gmail/callback", async (req, res) => {
  ...
  if (error) {
    res.redirect(`${config.webBaseUrl}/?oauth=error&message=${encodeURIComponent(String(error))}`);
    return;
  }
  ...
  const stateRes = await pool.query<{
    ...
    `SELECT user_id, expires_at FROM oauth_states WHERE state = $1 AND provider = 'gmail'`,
    [state],
  );
  ...
  try {
    ...
    // Upsert by user+provider+email
    const existing = await pool.query<{ id: string }>(
      `SELECT id FROM mailbox_connections
       WHERE user_id = $1 AND provider = 'gmail' AND email_address = $2`,
      [row.user_id, tokens.email],
    );
    ...
    if (existing.rows[0]) {
      ...
      await writeAuditLog({
        ...
        eventType: "mailbox_connected",
        details: { provider: "gmail", email: tokens.email, refreshed: true },
      });
    } else {
      await pool.query(
        `INSERT INTO mailbox_connections (
          id, user_id, provider, email_address, access_token_ciphertext,
          refresh_token_ciphertext, scope, status
        ) VALUES ($1,$2,'gmail',$3,$4,$5,$6,'active')`,
        [
          id,
          row.user_id,
          tokens.email,
      ...
      await writeAuditLog({
        ...
        mailboxConnectionId: id,
        eventType: "mailbox_connected",
        details: { provider: "gmail", email: tokens.email },
      });
    }

    res.redirect(`${config.webBaseUrl}/?oauth=success`);
  } catch (err) {
    const message = err instanceof Error ? err.message : "OAuth failed";
    res.redirect(`${config.webBaseUrl}/?oauth=error&message=${encodeURIComponent(message)}`);
  }
});
