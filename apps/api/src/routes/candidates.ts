...
candidatesRouter.post("/:id/post-civisight", async (req, res) => {
  if (!config.civisight.enabled) {
    res.status(403).json({
      error:
        "CiviSight posting is disabled. Set ENABLE_CIVISIGHT_POSTING=true to enable.",
    });
    return;
  }
  if (!config.civisight.adminToken) {
    res.status(400).json({ error: "CIVISIGHT_ADMIN_TOKEN is not configured" });
    return;
  }

  const existing = await getOwnedCandidate(req.params.id, req.user!.id);
  if (!existing || !["approved", "exported"].includes(existing.status)) {
    res.status(400).json({ error: "Candidate must be approved before posting" });
    return;
  }

  let item;
  try {
    item = buildExportItem(existing as CandidateExportRow);
  } catch (err) {
    res.status(400).json({
      error: err instanceof Error ? err.message : "Invalid export payload",
    });
    return;
  }

  const validationErrors = validateBeforePost(item.task);
  if (validationErrors.length) {
    res.status(400).json({ error: "Validation failed", details: validationErrors });
    return;
  }

  const response = await fetch(`${config.civisight.apiBaseUrl}/tasks`, {
    method: "POST",
    headers: {
      Authorization: `Bearer ${config.civisight.adminToken}`,
  ...
  if (!response.ok) {
    ...
    res.status(502).json({
      error: "CiviSight posting failed",
  ...
  const created = (await response.json()) as { id?: string; task?: { id?: string } };
  const createdId = created.id ?? created.task?.id ?? null;

  await pool.query(
    `UPDATE email_task_candidates
     SET status = 'posted', created_task_id = $1, updated_at = NOW()
     WHERE id = $2`,
    [createdId, req.params.id],
  );
  await writeAuditLog({
    userId: req.user!.id,
    mailboxConnectionId: existing.mailbox_connection_id,
    eventType: "task_posted_to_civisight",
    details: { candidateId: req.params.id, createdTaskId: createdId },
  });

  res.json({ ok: true, createdTaskId: createdId });
});

export const entitiesRouter = Router();
entitiesRouter.get("/", requireAuth, (_req, res) => {
