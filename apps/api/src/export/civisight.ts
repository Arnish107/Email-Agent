...
export const civisightTaskSchema = z.object({
...
export type CiviSightTaskPayload = z.infer<typeof civisightTaskSchema>;
...
export function buildExportItem(row: CandidateExportRow): ExportItem {
  ...
  const task = civisightTaskSchema.parse({
    title: row.title,
...
export function validateBeforePost(task: CiviSightTaskPayload): string[] {
  const errors: string[] = [];
